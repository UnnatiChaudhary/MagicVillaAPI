...
namespace MagicVillaAPI.Controllers.v2
{
    ...
    public class VillaAPIController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetV20()
        {
            ...
            {
                return Ok(new { source="cache", data = cacheDrivers , version="2.0", summary = "Study nicely" });
            }
            ...
            return Ok(new { source="database", data = villas, version="2.0", summary="Study nicely" });
        }

        [HttpGet("villas")]
        [MapToApiVersion("2.1")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> GetV21()
        {
            var cacheDrivers = _cacheService.GetData<IEnumerable<Villa>>("villas");
            if (cacheDrivers != null && cacheDrivers.Count() > 0)
            {
                return Ok(new { source = "cache", data = cacheDrivers, version = "2.1", summary = "Weather is nice" });
            }
            var villas = await _db.Villas.ToListAsync();
            var expiryTime = DateTimeOffset.Now.AddMinutes(2);
            _cacheService.SetData<IEnumerable<Villa>>("villas", villas, expiryTime);

            return Ok(new { source = "database", data = villas, version = "2.1", summary = "Weather is nice" });
        }
        ...
        public IActionResult GetVilla(int id)
        {
            ...
            if (Villa == null)
            {
                ...
                var expiryTime = DateTimeOffset.Now.AddMinutes(2);
                _cacheService.SetData<IEnumerable<Villa>>("villas", villas, expiryTime);
                return Ok(new { source = "database", data = villa, version = "2.0" });
            }
            else
            {
                return Ok(new { source = "cache", data = Villa , version = "2.0" });
            }
        }
        [HttpPost("villa")]
